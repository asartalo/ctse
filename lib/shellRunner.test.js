...
const race = require('./promises/race');
const wait = require('./promises/wait');
...
describe('shellRunner()', () => {
  ...
  describe('when running a long running task @slow', () => {
    beforeEach(() => {
      resolved = null;
      runner = shellRunner('ping', ['google.com'], runOptions);
      runner.run().then(val => {
        resolved = val;
      });
    });
    ...
    describe('when it is killed', () => {
      ...
      it('finally resolves', async () => {
        ...
        expect(resolved.command).to.equal('ping');
      });
    });
  });
});
